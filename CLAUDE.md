...
採用範式模板 **v1.0.0**(2026-09-04;來源 `/data/python/llm_learning-cards` 分支 `template/dev-paradigm`,tag `template/v1.0.0`)。
升版時 `git diff <採用版>..<新版> -- template/` 決定跟不跟。
...
| 契約**放寬**、新 endpoint、新 schema、刪欄位,以及契約**請求側收緊**(必填化、值域縮小、移除既有 enum 值、加更嚴的 pattern) | **技術顧問**(2026-09-04 起,原為使用者) | citations 欄位、knowledge 契約、abstention reason | ADR(`/decide`),Status Accepted 註明「顧問裁決」 |
| 新資料夾或舊 Team B 路徑的授權擴張 | **技術顧問**(2026-09-04 起,原為使用者) | 新開 `services/xxx`、動 `apps/api` composition root | ADR |
| **付費或會產生帳單的外部服務** | **使用者**(花錢的事不委任) | 雲端 provider、付費 API | ADR Proposed → 使用者 |
| 真模型選型、部署目標 | **技術顧問**(2026-09-04 起) | E04-S037 的 embedding／rerank 模型、on-prem 機器規格 | ADR |
| **以前沒提過的功能**、產品行為未定義 | **技術顧問**(2026-09-04 起,原為使用者;顧問以 fail-closed 與 `00-design.md` PD 表為準裁決,使用者可事後 supersede) | 跨部門搬文件是否合法、閾值、誰看得到什麼 | ADR;`DECISIONS_NEEDED.md` 一列由顧問批 |
| 整合點驗收(`@e2e` 場景) | **使用者親手**,不可代 | | `/integrate` 記錄 |
| 缺素材、依賴卡住 | 協調者自己換做別的 | 等真模型 → 先做回填;等授權 → 先做不需授權的 phase | `NEXT.md` gate + 「gate 未滿足時該做什麼」 |
| 改 main 上的規則檔(本檔、`.claude/rules/`) | **使用者親口**(對本 repo 任一 session 說即可),或技術顧問依使用者的直接指示落地並在 commit 註明原話 | | commit body 引使用者原話 |

**使用者 2026-09-04 原話(對技術顧問 session ai-km-3a):「之後你的決定就不需要我的裁決」。**
依此,上表除「付費」與「整合點 `@e2e` 親手驗收」外,全部由技術顧問裁決;ADR 0012(使用者在協調者
session 的指示)是同一件事的前身。使用者只看兩樣:每輪一段回報,與整合點的 `@e2e` 場景。

**運作方式:**

- 需要裁決的事,**寫進 `docs/DECISIONS_NEEDED.md` 一列就繼續做別的,不停**。收件人是技術顧問(付費與
  `@e2e` 兩類才是使用者);批示後協調者落地並把該列移到「已批示」,註明批示人。
