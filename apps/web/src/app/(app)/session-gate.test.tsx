...
const { mockReplace, mockRouter, mockPathname } = vi.hoisted(() => {
  ...
  // (which does not change identity across renders) — a fresh object per
  // call would spuriously re-trigger any effect that depends on `router`.
...
describe("SessionGate", () => {
  ...
  it("redirects to /login with the current path as returnUrl when there is no session", async () => {
    ...
    await waitFor(() => expect(mockReplace).toHaveBeenCalledWith("/login?returnUrl=%2Fdashboard"));
    expect(screen.queryByText("protected content")).not.toBeInTheDocument();
  });
