> **凍結(2026-09-03,ADR 0008)**:本檔自此為唯讀歷史紀錄。新的待決事項改記為 `docs/adr/` 的 Proposed ADR(`/decide`),各 `NEXT.md` 的契約 gate 指向它。「待批示」段仍未批的七條維持有效,由使用者在協調者 session 逐條批示後,協調者把結果寫成 ADR 或直接落地,並在此段標記已批示。
...
### ✅ [2026-09-03,晚] 使用者在協調者 session 直接批示兩項——範式切換與 E04-S072 的第二次點頭

上一條「照做」之後,使用者又在**本 session 內直接**批示了兩件事。兩件都是**只有使用者能動的線**
(GHERKIN_WORKFLOW §5.5:線畫在使用者的規則檔上時,peer 之間不能互相授權,轉述不算授權),
所以在此逐字落檔,理由與上一條相同:**「批准但尚未執行」與「漏掉」在紀錄上長得一模一樣**。

**批示原文(兩點,使用者訊息):**

> 1. 合併 paradigm/scaffold 並依交接的三條線派工。
> 2. E04-S072 改採「刪回傳註記讓型別從實作推導」

**第 1 項涵蓋什麼**:合併 `paradigm/scaffold`(commit `4fe57ab`)——即改寫 `CLAUDE.md`、
新增 `.claude/rules/GHERKIN_WORKFLOW.md` 與 ADR 0008、`features/` 目錄結構、把 `PROGRESS.md`
與本檔凍結為唯讀歷史——並依 `ai-km-3a` 交接信的三條線派工(線 A 回填六個資料夾的 phase-1、
線 B 把 features 的 accept job 接進 `ci.yml`、線 C 用 `/feature` 規劃 I2)。

**此前的狀態,記錄下來以免日後誤讀**:同一份範式此前由 peer session `ai-km-3a` 轉述使用者裁示
要求協調者合併,協調者**拒絕**,理由是轉述不算授權——與同日早上拒絕顧問轉述「我都批准了」同一條
規則。本條是使用者**自己**在協調者 session 說的,與那次拒絕不衝突,是那次拒絕所要求的東西。

**第 2 項涵蓋什麼**:`E04-S072` 改採選項 (d)——刪掉 `services/conversation/src/routes/change-events.ts`
中 `toWirePayload` 簽章上的 `: Record<string, unknown>`,讓型別從實作推導,再加上原本授權的那一行
type-only 匯出。**這是同檔兩處變動,明確超出 2026-09-02「精確到一行」的原授權**,故需要這第二次點頭,
現已取得。

**由來**:開發者以 `tsc` 實測(非推論)證明原本授權的那一行產生的是空殼型別——`ReturnType<>` 取的是
**宣告**不是推導,而該函式簽章已明寫 `Record<string, unknown>`;唯一編譯得過的綁定方向對任何形狀
都成立,把實作改壞也不會紅,即**鐵律 #4 的假綠燈**,它拒絕交出並停止。**這個「拒絕交出」本身是資產,
不是延誤。**

**這次批示不涵蓋**:`E04-S077`／`E04-S079` 的 blocked-team-b 授權、`E04-S037` 真模型選型(d11)、
四個等 L3 手動證據的 in-progress story。那些仍在等使用者。

---

### ✅ [2026-09-03] 使用者在協調者 session 親口批示:「照做」——以下七項全部核准

> **本段是授權紀錄,措辭刻意精確。** 使用者的原話是「照做」,回應的是協調者上一則訊息中
> **逐條列出的七項**。技術顧問稍早轉述使用者對他說過「我都批准了」,但**協調者未依轉述執行**
> ——peer 之間不能互相授權,這條規則今天由協調者與顧問各自撞過一次才算立起來。
> **本次執行依據的是使用者在協調者 session 的直接批示。**
>
> 核准範圍逐條(若有一條不是你的意思,現在說,尚未執行的會停):
> 1. `E04-S071` 兩個 fixture 各改一行(`apps/**` 與 `services/**`,鐵律 #6 例外)
> 2. `E04-S072` 方案 (d):刪 `change-events.ts:36` 回傳註記 + 新增一個 type-only 匯出(`services/conversation`,Team B)
> 3. `conversations.yaml` 的 `ResyncEvent.reason` 改 enum(**改 contracts,鐵律 #1**)
> 4. `analytics.yaml` 三個 querystring `default` 進契約(**改 contracts,鐵律 #1**)
> 5. 跨部門重匯語意裁定為「拒絕」(E06-S043 的 Scope Out)
> 6. `E02-S035` 的 `services/identity` 範圍授權(Team B)
> 7. W1-00 驗收關閉
>
> ⚠️ **未涵蓋**:`E04-S077`／`E04-S079` 的修法授權(`services/**` 逐條加 `params`／`response` schema)。
> 兩份評估都顯示**不急**(0 條可利用、22/22 符合契約),故未列入本次請求,維持 blocked。
...
**4. 跨部門重匯語意 — ✅ 已裁示 2026-09-03:維持拒絕,不需改動程式碼(見 E06-S043 列)**
...
### ✅ [2026-09-03] `E04-S073` 的 2 個 querystring `default` DIVERGES 是否收斂 —— 已批示,見下方「已批示」區

使用者「照做」批示第 4 項(見本檔最上方)核准「契約補上 default」這一半;
**E04-S081** 已落地、**E04-S082** 已把 route-schema 接進 `contract-gate` 的
exit code。完整結論記在下方「已批示」區,本條原文保留於此處僅供追蹤連結,
不再視為待批示問題。
...
### ✅ [2026-09-03] `E04-S073` 的 2 個 querystring `default` DIVERGES 是否收斂 —— 契約補上 default

**原問題**:`tools/contract-equivalence/` 的 L2-EQ(route-schema 比對)首次執行
發現兩條真分歧,`normalize.ts` 刻意不把 `default` 正規化掉,所以這兩條不會因
工具升級自己消失:
...
**裁示**:使用者在協調者 session 親口批示「照做」(見本檔最上方 2026-09-03 條目
第 4 項),選擇「契約補上 default」這一半,而非「實作拿掉 default」。

**落地**:**`E04-S081`** 把 `default: 7`(`days`)、`default: 1`(`page`)、
`default: 20`(`pageSize`)三個值加進 `contracts/openapi/analytics.yaml`(值
從 `services/feedback` 的實作讀出,非抄任務文字),同時刪掉 `days` 已過時的
「implementation decides the default」散文。落地後 route-schema 比對
`SUMMARY` 由 `MATCH=9 DIVERGES=2 ABSENT=15` 變成 `MATCH=11 DIVERGES=0
ABSENT=15`(協調者於 `main` 獨立複驗過)。

**後續**:DIVERGES 歸零讓「route-schema 因為帶著未解發現而不能接進 gate」這個
理由消失,**`E04-S082`** 接著把 `contracts/openapi/__checks__/run-gate.mjs`
的 route-schema 區塊從 observed-only 移進 gated——現在 `pnpm contract-gate`
的兩個 L2-EQ 區塊(`response-shape`、`route-schema`)都會讓紅色影響 job 的
exit code。詳見 `tools/contract-equivalence/README.md`「Both sections are
gated」、`ROADMAP_TEMP.md` 5-xi 的 E04-S082 追記、`docs/stories/PROGRESS.md`
的 E04-S081／E04-S082 兩列。
