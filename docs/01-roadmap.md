...
| 欄位 | 值 |
|---|---|
| 目前階段 | I2 · web 提問(I1 已通過 2026-09-03)。**I2 五塊已完成 4 塊**:`06-retrieval/phase-2`(09-04)、`07-generation`、`05-ingestion`、`03-conversation`(09-05)。**只剩 `11-app-shell/phase-2`(引用可點、開原文段落面板)** |
...
**升版目標:v1.4.1(tag 待出)。** ※ 2026-09-05 更新:本段先後寫過 v1.3.2、v1.3.4;
...
> ### ⚠️ 升版前必須先解掉的一條(2026-09-05,顧問 ai-km-1b 回報)
>
...
>
> 顧問已要對方給 `--verbose` 輸出。**這條沒解不升版。**
>
> 這是 `docs/PITFALLS.md` **坑 18** 的同一個家族的又一種:那裡是「設定沒被讀到,
> 而輸出與『沒有設定』一模一樣」;這裡是「**同一份設定被兩個版本讀出完全不同的世界**」。
> 共同點是:**數字本身不會告訴你它是不是量對了東西。**
