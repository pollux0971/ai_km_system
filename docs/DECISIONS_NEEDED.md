...
| 19 | 2026-09-05 | **規則字面與實務不一致,而實務已經跨多個 phase 一致地這樣做了。** GHERKIN_WORKFLOW §6 的表格把 `.feature` 列在「使用者(PO)能改」欄,測試 agent 欄只列 `features/steps/**`、`*.test.ts`;§3 的生命週期也把「寫 `.feature`」放在 `/feature`(提案→確認→寫檔)裡。**但實際上**每個 phase 的 `.feature` 都是測試 agent 在「先寫紅」那個 commit 裡直接建立的——`06-retrieval/phase-2`(`862452d`)、`07-generation/phase-2`(`3ca3820`)、`05-ingestion/phase-2`(`d701cdd`)都是同一個模式,由 05 的獨立驗收 session 逐 commit 查出並指出。<br>**兩個方向都合理,要你選一個**:(a) 承認實務——`/feature` 管的是「這個 phase 該不該存在」(寫進 `FEATURE.md` 的 phase 表),phase 表上已核准的 phase,其 `.feature` 由測試 agent 在先寫紅時建立;`.feature` **既有內容的修改**仍走 `/feature`。(b) 照字面執行——每個 phase 的 `.feature` 都要先走一次 `/feature` 提案確認。<br>我傾向 (a):它讓「使用者把關的是能力邊界,不是每個 phase 的測試文字」這件事說得通,而 (b) 會在每個 phase 前面加一道對你的等待。 | 規則檔 | 我不自己改規則檔(ADR 0013 §4)。**在你裁定前我照現行實務繼續**,並在每次 phase-done 的紀錄裡標明——不假裝這個落差不存在 | 不擋任何事;擋的是規則與實務哪一個才算數 |
| 18 | 2026-09-05 | **「共用檔只有協調者改」這條規則連續兩輪被同一個機械理由跨過,我不想第三次才處理。** 兩次都是開發 agent 在 `feat` commit 裡動了共用檔:02-authorization phase-2b 補一行 `deniedScopeKeys: []` 到 `features/steps/integration.steps.ts`(必填化的機械後果);07-generation phase-2 加一行 `"@ai-km/service-generation": "workspace:*"` 到 `apps/api/package.json`(composition root import 的機械後果,沒有它 typecheck 直接紅)。兩次內容都無害、都主動回報、都有前例(06-retrieval phase-2 做過一模一樣的事)。<br>**我上一輪說「下次共用檔的改動要由協調者自己開一個 commit」——然後這一輪它又發生了,而我還是接受了。** 一條說了不執行的規則比沒有這條規則更糟(PITFALLS 坑 1 的形狀)。<br>**建議的收斂**(要改 `.claude/rules/GHERKIN_WORKFLOW.md` §6,所以是你的):加一句「**被 import／型別強制的機械後果**(移除它會讓 `pnpm typecheck` 當場紅)不算判斷,開發 agent 可以改共用檔的那一行,但必須在 commit body 點名;**其餘共用檔改動仍然只有協調者能動**」。判準跟契約那條同一個形狀:**可機械驗證的後果 vs 判斷**。 | 規則檔 | 我不自己改規則檔(ADR 0013 §4 明訂規則檔仍要你親口)。你說「加」我就加;你說「不加,以後真的退回」我就照那樣執行,不再接受 | 不擋任何事;擋的是這條規則繼續空轉 |
