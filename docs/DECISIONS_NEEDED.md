...
|---|---|---|---|---|---|
| 17 | 2026-09-04 | **回填 12/12 唯一還缺的東西:三個資料夾的 `@manual`／`@e2e` 場景要你親眼走一次**(08、10、11)。自動那半三個都已全綠並各自做過反向驗證。<br>**跑法**:`pnpm --filter @ai-km/admin dev`(:3001)+ `pnpm --filter @ai-km/web dev`(:3000)。<br>**10-admin-console**(`@e2e @manual`):以 IT 管理員登入,依序打開 部門管理／群組管理／連接器管理／系統健康儀表板 → 「每頁都渲染自己的清單而不是錯誤畫面」,且「資料就是自動場景印出來的那一份」。<br>**08-knowledge-management**(2×`@manual` + 1×`@e2e @manual`):知識庫頁用一行 dev 指令開得起來;選檔上傳後檔案留在待處理清單(E03-S028 競態);上傳後文件出現在清單裡。**注意這一層目前整個是 mock**(鐵律 5),你確認的是畫面行為,不是後端接上了。<br>**11-app-shell**(`@e2e @manual`):同一個人開兩個瀏覽器視窗,其中一個開新對話並送訊息 → 另一個視窗的 歷史對話 rail 不重新整理就列出它;把 API 停掉 → 兩個 header 都顯示「同步連線中斷,重新連線中…」 | 整合點驗收(`@e2e`／`@manual`) | 依 ADR 0013,這是**仍然只有你能做**的兩件事之一(另一件是花錢)。§5.4:任何檢查都構不到「有人看過並接受」。做不到的地方請直接說哪一步卡住——那比通過更有價值 | 回填完成定義(12/12)。**不擋 I2**——I2 的五塊都不依賴這三條 |
