...
## 追加段(2026-09-05,技術顧問 ai-km-1b 裁決):anydoc「刪減」兩案**都不做**

協調者曾把「要不要刪減 anydoc」掛在使用者那級。顧問指出**那是工程取捨、屬顧問**,收回並裁決。

### (a) 只裝需要的平台 binary —— **不做**

**理由是機制,不是偏好**:pnpm 安裝時本來就只裝 `os`/`cpu` 相符的 `optionalDependencies`,
別的平台的 binary **根本不會進 `node_modules`**。lockfile 留著全部平台不佔任何東西;
砍掉它們反而**把 on-prem 目標釘死在 linux x86_64**——那是部署決定,屬 I9 runbook,
不該由一個省空間的動作偷偷做掉。

**但這條結論本身要量一次(§5.3:機制要用量的,不要用讀的)**:
`05-ingestion/phase-2c` 的 evidence **必須貼 `ls node_modules/@firecrawl/` 的實際輸出**,
只該出現 linux-x64 那一包。**若不是,回來重裁**——上面那段推論就不成立。

上一節「另外三件」第 1 條(離線鏡像要連 7 個平台子套件一起鏡像)**不變**,
它講的是 mirror,不是本機安裝。

### (b) fork anydoc 刪掉 hosted OCR 原碼 —— **不做**

理由:那是 Rust 專案,本 repo 一行 Rust 都沒有;fork 等於長期維護一個我們不會編的東西。
硬禁令留在紀律 + 測試那一層。

**但顧問把守門的斷言形狀收緊了,這一段比「不 fork」重要**:

第五個條件說「要有一條會紅的檢查確認 `ocr` 沒有被設成 `'hosted'`」。
**只斷言「給掃描版 PDF 會拋 `NeedsOcr`」不夠**——把實作改成「先送 hosted、拿到結果再拋錯」,
那條斷言**仍然會綠**,而文件已經離開這台機器了。

所以斷言要對著**會變的量**:給一份掃描版 PDF,斷言拋 `NeedsOcr`
**而且 fetch / undici 的 mock 呼叫次數為 `0`**。
「沒有送出網路請求」才是這條真正要守的性質;「有拋錯」是它的副作用(§5.2)。
