...
| E03 AI Conversation Experience | 33 | 31 | 1 | 0 | 0 | 1 |
...
| E03-S032 | done | story/E03-S032-message-retry-ux | [E03-S032.md](E03-S032.md) | DEV 完成,待獨立審核(Message retry UX,SOURCE_BASELINE 無此 story 章節(E03 條目在 S30 結束),範圍依本 story 通用 Functional AC 5 稽核既有四種重試機制得出,不需 advisor;修正 `handleRetryStream` 遺失 `reviseTarget`/`answerState` 的真實 bug——重試一次失敗的 regenerate 原本會產生重複訊息而非更新原訊息,直接違反 AC 5;`stream-disconnected` 的同形狀缺口因目前不可達,刻意不動;0 次 FIX 循環;typecheck/lint/build/454 unit/32 個目標 E2E 皆綠,force 全量兩輪皆綠) |
| E03-S033 | todo | | | |
