...
| E07 Maintenance Assistant Experience | 25 | 3 | 1 | 0 | 0 | 21 |
...
| E07-S004 | done | story/E07-S004-error-code-search-ui | [E07-S004.md](E07-S004.md) | Error-code search UI,延續 S002-S003 共用表單設計加第三個欄位;與 S002 純選擇器不同,建立真正的搜尋窄化(文字框即時依代碼/描述關鍵字過濾 select 選項),因為 story 標題明確寫「search」而非「selector」;`errorCode` 選填但有值須驗證(參照 ERROR_CODE_OPTIONS 固定清單,同 equipmentId 驗證邏輯,只是放寬允許空值),與 serialNumber 完全不驗證不同;搜尋窄化後若已選代碼被篩掉會自動清空,避免選單顯示畫面上看不到的選取值;開發中發現 Playwright getByLabel 預設子字串比對、Testing Library getByLabelText 預設精確比對,同一字串在單元測試通過、E2E 測試因誤中「搜尋錯誤代碼」子字串而逾時,修正為 exact:true 並記錄；1 次 FIX 循環,typecheck/lint/940 unit/159 E2E/build 全綠 |
| E07-S005 | todo | | | |
| E07-S006 | todo | | | |
| E07-S007 | todo | | | |
| E07-S008 | todo | | | |
