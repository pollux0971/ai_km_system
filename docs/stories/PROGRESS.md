...
| E05-S015 | done | story/E05-S015-text-knowledge-input | [E05-S015.md](E05-S015.md) | Text knowledge input;`KnowledgeBaseDocument` 新增選填 `content`,真的儲存輸入文字(與 S011-S014「不假裝擁有不存在的資料」同一原則的相反應用——這次資料真實存在,誠實儲存);`sizeBytes` 為真實計算的 UTF-8 位元組數(`new Blob(...).size`),與 S014 URL 匯入省略此欄位形成刻意對照;標題/內容雙欄位分離,鏡射 createKnowledgeBase 的 name/description 分離;空白內容明確拒絕(與 S008 boundPrompt 允許空白的判斷不同,理由記錄);`KnowledgeDocumentTextInput` 第三個獨立元件;0 次 FIX 循環(全部 gate 一次通過);typecheck/lint/697 unit/137 E2E/build 皆綠;待獨立審核 |
| E05-S016 | todo | | | |
| E05-S017 | todo | | | |
| E05-S018 | todo | | | |
| E05-S019 | todo | | | |
