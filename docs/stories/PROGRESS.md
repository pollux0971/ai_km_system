...
| E05 Knowledge Management Experience | 31 | 12 | 1 | 0 | 0 | 18 |
...
| E05-S013 | done | story/E05-S013-folder-upload | [E05-S013.md](E05-S013.md) | Folder upload;純延伸 S011/S012 既有的 `KnowledgeDocumentUpload` 元件(未新增任何 lib export);新增第二個獨立 `<input webkitdirectory>`(callback ref 命令式設定,避免 `any` 型別斷言),與既有輸入共用同一套選取/預覽/移除/上傳/telemetry 管線;`displayName()` 優先採用 `webkitRelativePath` 保留資料夾結構,避免同名檔案在不同子資料夾下無法區分;1 次 FIX 循環(Playwright 拒絕對 webkitdirectory input 使用 buffer-based setInputFiles,改用真實臨時資料夾反而讓驗證更真實——真正的瀏覽器資料夾選取產生真實的 webkitRelativePath);typecheck/lint/654 unit/135 E2E/build 皆綠;待獨立審核 |
| E05-S014 | todo | | | |
| E05-S015 | todo | | | |
| E05-S016 | todo | | | |
| E05-S017 | todo | | | |
