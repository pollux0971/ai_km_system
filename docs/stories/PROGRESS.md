...
| E05-S014 | done | story/E05-S014-url-import | [E05-S014.md](E05-S014.md) | URL import;`KnowledgeBaseDocument.sizeBytes` 改為選填(URL 匯入無真實位元組數可回報,不捏造佔位數字);新增獨立 `addKnowledgeBaseDocumentFromUrl`(格式驗證+http(s)協定白名單,不與檔案上傳共用同一驗證邏輯);`KnowledgeDocumentUrlImport` 獨立元件(文字輸入+明確送出,非 KnowledgeDocumentUpload 的第三種模式);顯示具體錯誤訊息(刻意偏離 S009 用固定通用字串的既有做法,有明確理由記錄);2 次 FIX 循環(`type="url"` 原生驗證攔截 submit 導致自訂驗證/錯誤訊息完全被繞過,改用 type="text";E2E route-announcer 碰撞,沿用 S003 既有先例修正);typecheck/lint/676 unit/136 E2E/build 皆綠;待獨立審核 |
| E05-S015 | todo | | | |
| E05-S016 | todo | | | |
| E05-S017 | todo | | | |
| E05-S018 | todo | | | |
