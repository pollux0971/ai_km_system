...
| E05 Knowledge Management Experience | 31 | 11 | 1 | 0 | 0 | 19 |
...
| E05-S012 | done | story/E05-S012-multi-file-upload | [E05-S012.md](E05-S012.md) | Multi-file upload;純延伸 S011 既有的 `KnowledgeDocumentUpload` 元件與 `addKnowledgeBaseDocument`(未新增任何 lib export);選取採累加而非取代,鏡射 `MessageComposer`/`FileAttachmentPicker`(E03-S008)既有先例;循序(非平行)逐檔呼叫,明確分析並拒絕依賴 microtask 排程巧合的平行呼叫方案;每個檔案獨立單位,部分失敗不影響其他檔案,失敗的留在清單供重試;telemetry 每檔各自一組 attempt/success/failure;2 次 FIX 循環(RTL/Playwright `exact` 選項語意誤用;E2E `getByRole("listitem")` 誤命中 app shell 導覽項目,修正為 `aria-label` 限定範圍);typecheck/lint/646 unit/134 E2E/build 皆綠;待獨立審核 |
| E05-S013 | todo | | | |
| E05-S014 | todo | | | |
| E05-S015 | todo | | | |
| E05-S016 | todo | | | |
