...
| 3b | **瀏覽器端不再有任何能自己產生助理回覆的碼或型別。** | I2 | todo | |

**phase-3b 的來源(2026-09-05,技術顧問 ai-km-1b 裁決;`DECISIONS_NEEDED` #47)**

`03-conversation/phase-4` 收緊 `role` enum 之後,**`apps/web` 編譯不過**——不是測試紅,
是 **Next build 失敗**。根因:`receiveAssistantReply` 的**函式本體、匯出、型別依賴,
以及約 50 條專屬測試**全部原封不動留著,`phase-3` **只是繞開了呼叫路徑**。

**ADR 0017 第二步第 1 件「apps/web 切到 server 生成,移除瀏覽器端生成(E03-S010)」的
「移除」從來沒有真的做。** 顧問原話:

> 「#47 我的切法有洞,你說得對:『**停止呼叫**』與『**移除**』是兩層,我只寫了前者。」

當初把四件事拆成兩個 phase 依序是對的(目的是「web 還在送時不能先拒絕」,用順序就達到),
但拆的時候沒有人注意到第 1 件裡有兩層。後者掉在兩個 phase 中間。
ADR 0017 警告的空窗真的出現了,只是形狀不同:**UI 不送了,但型別仍然假設它能送。**

**為什麼編號是 `3b` 而不是新的整數**:它補的是 `phase-3` 沒做完的那一半,不是新能力
(先例:`05-ingestion/phase-2b`)。而且 `phase-4`／`phase-5` 已經被順延過兩次
——「編號會動,意圖不會動」那句就是為了這種時候。

**範圍**:刪 `receiveAssistantReply` 的本體、匯出、型別依賴,與它那約 50 條專屬測試
(**A 類,依顧問 2026-09-05 的預先確認刪**,commit body 要**列清單**)。

**反向驗證(顧問指定,不是 typecheck 綠就算)**:
**`grep -rn 'receiveAssistantReply'` 全 repo 零命中**才算數。理由:typecheck 綠只證明
「現在沒有人用錯」,零命中才證明「**沒有東西可以再用它**」——這個 phase 的意圖句要的是後者。

**它是 `03-conversation/phase-4` 的新 gate。** **級別:標準。**
