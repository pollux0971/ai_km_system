 ...
 * ## 判定
 *
 * 一句正規化後的句子如果出現在兩個以上分組裡:
 * - 已經在 `common.steps.ts` 定義過 → 合法共用,列出但不算失敗
 * - 沒有定義在 `common.steps.ts` → **exit 1**,列出句子與所有出現的檔案
 *
...
const COMMON_STEPS_FILE = resolve(FEATURES_ROOT, "steps/common.steps.ts");
...
/** 從 common.steps.ts 抽出所有 Given(...)/When(...)/Then(...) 的第一個字串參數(cucumber expression pattern) */
function commonStepPatterns(file: string): string[] {
...
const commonNormalized = new Set(commonStepPatterns(COMMON_STEPS_FILE).map(normalizePattern));

const crossFolder = [...bySentence.entries()].filter(([, occ]) => new Set(occ.map((o) => o.group)).size > 1);

const covered = crossFolder.filter(([norm]) => commonNormalized.has(norm));
const uncovered = crossFolder.filter(([norm]) => !commonNormalized.has(norm));
...
if (covered.length > 0) {
  console.log(`\nstep-dup: ${covered.length} sentence(s) shared across folders and already defined in common.steps.ts (OK):`);
  for (const [norm, occ] of covered) {
    const groups = [...new Set(occ.map((o) => o.group))];
    console.log(`  - "${norm}" — ${groups.join(", ")}`);
  }
}

if (uncovered.length === 0) {
  console.log("\nstep-dup: PASS — no cross-folder step sentence collision outside common.steps.ts.");
  process.exit(0);
}

for (const [norm, occ] of uncovered) {
  console.log(`\nstep-dup: sentence used in more than one folder, but NOT defined in common.steps.ts:`);
  console.log(`  sentence: "${norm}"`);
  for (const o of occ) console.log(`  - ${o.group} :: ${relative(REPO_ROOT, o.file)} :: ${o.raw}`);
}
console.log(
  `\nstep-dup: FAIL — ${uncovered.length} step sentence(s) collide across folders. ` +
    `Move each one into features/steps/common.steps.ts (coordinator-owned) so cucumber binds it once, on purpose.`,
);
process.exit(1);
