 ...
 * Same reasoning as `services/model-gateway/src/plugin-types.ts` and
...
export interface RetrievalAuthContext {
...
export function requestAuth(request: FastifyRequest): RetrievalAuthContext | undefined {
  return (request as unknown as { auth?: RetrievalAuthContext }).auth;
}
